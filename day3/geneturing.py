...
import blast_Eutils  # Assuming blast_Eutils is a custom module for BLAST operations
# ## 2. Configuration
...
def get_answer(answer: str, task: str) -> str:
    ...
    if task in ["Gene location", "SNP location"]:
        ...
        if "chr" not in answer:
            answer = "chr" + answer
        return answer
    elif task in ['Gene alias',"Gene disease association","Gene name conversion","Gene SNP association"]:
        return answer.upper()
    elif task in [
...
MAX_TOOL_CALL_TURNS = 9 # Max iterations of LLM <-> Tool
...
with mlflow.start_run(run_name="gene_turing_run_with_blast_Eutils"):
    # Log basic config
    ...
    imag_path=f"{DATASET}_scores_with_blast_Eutils.png"
    plot_scores_by_task(
        task_scores, overall_score, imag_path=imag_path
    )
    # Log the image
